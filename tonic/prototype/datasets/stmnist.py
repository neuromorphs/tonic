...
class STMNISTFileReader(IterDataPipe[EventSample]):
    ...
    def __init__(
            ...
            dtype: Optional[np.dtype] = np.dtype([("x", int), ("y", int), ("t", int), ("p", int)]),
            ) -> None:
        self.dp = dp
        self.dtype = dtype
    ...
    def __iter__(self) -> Iterator[EventSample]: 
        for fname, fdata in self.dp:
            yield {'events': self._mat_to_array(fdata), 'target': self._get_target(fname)}
    ...
    def _datapipe(self) -> IterDataPipe[EventSample]:
        ...
        if self.transforms: 
            ...
            if self.target_transform:
                dp = Mapper(dp, self.target_transform, input_col="target", output_col="target")
        return dp
