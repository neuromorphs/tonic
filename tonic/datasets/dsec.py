...
from tonic.download_utils import check_integrity, download_and_extract_archive, download_url, list_files
...
#             "zurich_city_15_a",
    ...
    def __init__(self, save_to, train=True, transform=None, target_transform=None):
        super(DSEC, self).__init__(save_to, transform=transform, target_transform=target_transform)
    ...
    def __getitem__(self, index):
        ...
        # events
        events_left_file = h5py.File(os.path.join(base_folder, "events_left", "events.h5"))["events"]
        events_left = np.column_stack((events_left_file['x'][()], events_left_file['y'][()], events_left_file['t'][()], events_left_file['p'][()]))

        events_right_file = h5py.File(os.path.join(base_folder, "events_right", "events.h5"))["events"]
        events_right = np.column_stack((events_right_file['x'][()], events_right_file['y'][()], events_right_file['t'][()], events_right_file['p'][()]))
        
        # images
        images_rectified_left_filenames = list_files(os.path.join(base_folder, "images_rectified_left"), ".png", prefix=True)
        images_left = np.stack([np.array(Image.open(file)) for file in images_rectified_left_filenames])
        
        images_rectified_right_filenames = list_files(os.path.join(base_folder, "images_rectified_right"), ".png", prefix=True)
        images_right = np.stack([np.array(Image.open(file)) for file in images_rectified_right_filenames])

        with open(os.path.join(base_folder, "image_timestamps", f"{recording}_image_timestamps.txt")) as f:
            image_timestamps = [int(line) for line in f.readlines()]

        data = ((events_left, events_right), (image_timestamps, images_left, images_right))
        ...
        if self.train:
            # ground truth
            disparity_event_filenames = list_files(os.path.join(base_folder, "disparity_event"), ".png", prefix=True)
            disparity_events = np.stack([np.array(Image.open(file)) for file in disparity_event_filenames])

            disparity_image_filenames = list_files(os.path.join(base_folder, "disparity_image"), ".png", prefix=True)
            disparity_images = np.stack([np.array(Image.open(file)) for file in disparity_event_filenames])

            with open(os.path.join(base_folder, "disparity_timestamps", f"{recording}_disparity_timestamps.txt")) as f:
    ...
    def _check_exists(self):
        for recording in self.recordings[self.train_or_test]:
            ...
            if self.train: filenames += self.target_filenames # add ground truth files when train=True
            for filename, extension in filenames:
                if check_integrity(os.path.join(self.location_on_system, recording, filename, f"{recording}_{filename + extension}")):
                    continue
