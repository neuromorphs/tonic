...
def to_frame_numpy(
    events, sensor_size, ordering, time_window=None, spike_count=None, time_bin_count=None, event_bin_count=None, overlap=0., include_incomplete=False, merge_polarities=False
):
    ...
    if not sum(param is not None for param in [time_window, spike_count, time_bin_count, event_bin_count]) == 1:
        raise ValueError("Please assign a value to exactly one of the parameters time_window, spike_count, time_bin_count or event_bin_count.")
    ...
    if time_window:
        ...
        event_slices = slice_by_time_bins(events, ordering, time_bin_count, overlap=overlap)
    elif event_bin_count:
        event_slices = slice_by_event_bins(events, ordering, event_bin_count, overlap=overlap)
