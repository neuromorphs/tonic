...
def plot_events(events, sensor_size, ordering, frame_time=25000, repeat=False):
    ...
    transform = transforms.Compose([transforms.MaskIsolated(time_filter=20000),
...
def plot_frames(frames, frame_time, repeat=False, vmax=255):
    ...
    return animation.FuncAnimation(fig, animate, frames=frames.shape[0], interval=frame_time/1000, blit=True, repeat=repeat)