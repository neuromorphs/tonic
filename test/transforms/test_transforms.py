...
class TestTransforms(unittest.TestCase):
    ...
    def testTimeReversalSpatialJitter(self):
        ...
        transform = transforms.Compose(
            [
                transforms.TimeReversal(
                    flip_probability=flip_probability, multi_image=multi_image
                )
            ]
        )
        ...
        self.assertFalse((events[::-1, 2] == self.original_events[:, 2]).all())
        self.assertTrue((events[::-1, 3] == self.original_events[:, 3]).all())
