...
@pytest.mark.parametrize("n, randomize_n, randgen_seed", [(100, True, 32), (100, False, 2), (0, False, -1)])
def test_transform_uniform_noise(n, randomize_n, randgen_seed):
    orig_events, sensor_size = create_random_input()
    # Random generator with a fixed seedto get the same random n generated
    # by the function call. 
    if randgen_seed>=0:
        randgen_fun = np.random.default_rng(randgen_seed)
        randgen_test = np.random.default_rng(randgen_seed)
        transform = transforms.UniformNoise(sensor_size=sensor_size, n=n, randomize_n=randomize_n, randgen=randgen_fun)
    else:
    ...
    rand_n = n
    if randgen_seed>=0 and randomize_n:
        rand_n = randgen_test.integers(n+1)
