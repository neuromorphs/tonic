...
@pytest.mark.parametrize("n, randomize_n, randgen_seed", [(100, True, 32), (100, False, None), (0, False, None)])
def test_transform_uniform_noise(n, randomize_n, randgen_seed):
    orig_events, sensor_size = create_random_input()
    # Random generator to get the same random n generated
    # by the function call using a seed. 
    randgen = np.random.default_rng(randgen_seed) if randgen_seed else None

    if randgen_seed:
        transform = transforms.UniformNoise(sensor_size=sensor_size, n=n, randomize_n=randomize_n, randgen=randgen)
    else:
        transform = transforms.UniformNoise(sensor_size=sensor_size, n=n, randomize_n=randomize_n)

    events = transform(orig_events)

    # Redefining the generator with the same seed.
    # In this way, we get the same number generated by the function
    # call.
    rand_n = n
    if randgen_seed and randomize_n:
        randgen = np.random.default_rng(randgen_seed)
        rand_n = randgen.integers(n+1)
    
    assert len(events) == len(orig_events) + rand_n
